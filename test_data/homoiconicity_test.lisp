...
;;; Run with: ./build/Darwin/arm64/basis.debug -f test_data/homoiconicity_test.lisp

					; ---- s-expressions inside tensor literals ----
					; expressions are evaluated when the tensor is built, not at parse time
(define x 3)
...
(print [[1 x] [x (+ x 1)]])
					...
					; a quoted tensor literal is just a (make-tensor ...) list, not a tensor yet
(define code '[1 2 3])
(print code)
...
(print (eval template))
					...
					; the [a b] literal becomes (make-tensor a b) which is evaluated each call
(define make-row (lambda (a b) [a b]))
(print (make-row 5 6))
					; => [5 6]

(print (make-row 100 200))
					; => [100 200]
