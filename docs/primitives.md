...
| `defvar`       | `(defvar count 0)`                 | Define a global variable (CL style) - global vars should be in *x1* style              |
...
| `make-tensor` | `(make-tensor 3 2 1)`       | `[3 2 1]`          | Create rank-1 tensor (template form of '[3 2 1])                 |
...
| `norm`      | `(norm [3 4])`          | `5`         | Euclidean norm (length) -> scalar (Measures distance from origin) |
| `norm2`     | `(norm2 [3 4])`         | `25`        | Norm squared (length squared) -> scalar (used in optimization)    |
